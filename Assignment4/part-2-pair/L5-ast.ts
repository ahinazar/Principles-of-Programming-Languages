...
const parseAppExp = (sexps: any[]): AppExp | Error =>{
    return safeFL((cexps: CExp[]) => makeAppExp(first(cexps), rest(cexps)))(map(parseCExp, sexps));
}
...
export const parseLitExp = (sexps: any[]): AppExp | LitExp | Error =>
    (isError(sexps))? sexps:
    (second(sexps).length === 3 && second(sexps)[1]==='.')? safeFL(parseAppExp)(["cons",second(sexps)[0],second(sexps)[2]]):
