...
;; <pair-te>      ::=  // pair-te(leftTE: te, rightTE : te)
...
import { isArray, isBoolean, isEmpty, isString, makeLitExp } from './L5-ast';
...
/*
;; expected structure: for proc (<params> -> <returnte>) or for pair : (param1 * param2)
...
const parseCompoundTExp = (texps: any[]): ProcTExp | PairTExp | Error => {
    if ( first(texps) === "Pair" && texps.length === 3 ) {
        const exp1 = parseTExp(texps[1]);
        const exp2 = parseTExp(texps[2]);
        return isError(exp1) ? exp1 :
                isError(exp2) ? exp2 :
                makePairTExp(exp1,exp2);
    }
...
export const unparseTExp = (te: TExp | Error): string | Error => {
    ...
    const up = (x: TExp | Error): string | string[] | Error =>
        ...
        isPairTExp(x)? ['Pair', unparseTExp(x.leftTE), unparseTExp(x.rightTE)]:
        isLitTExp(x) ? 'literal' :
...
export const matchTVarsInTE = <T1, T2>(te1: TExp, te2: TExp,
    ...
    (isPairTExp(te1) || isPairTExp(te2))?  matchTVarsinPair(te1, te2, succ, fail):
...
const matchTVarsinPair = <T1, T2>(te1: TExp, te2: TExp,
        succ: (mapping: Array<Pair<TVar, TVar>>) => T1,
        fail: () => T2): T1 | T2 =>
