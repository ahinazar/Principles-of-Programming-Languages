...
import { map, zip, zipWith, reduce } from 'ramda';
import { isAppExp, isBoolExp, isDefineExp, isEmpty, isIfExp, isLetrecExp, isLetExp, isNumExp,
         isPrimOp, isProcExp, isProgram, isStrExp, isVarRef, parse, unparse,isLitExp,
...
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, ProcTExp, StrTExp, TExp, makeLitTExp } from "./TExp";
...
export const typeofLitExp = (lit: LitExp, tenv: TEnv): TExp | Error => makeLitTExp();
...
export const typeofDefine = (exp: DefineExp, tenv: TEnv): TExp | Error => {  
    ...
    const constraint = checkEqualType(valTE, exp.var.texp, exp);
...
export const typeofProgram = (exp: Program, tenv: TEnv): TExp | Error => 
    typeofExps(exp.exps,reduce((acc,curr)=>isDefineExp(curr)?makeExtendTEnv([curr.var.var],[curr.var.texp],tenv) : acc ,makeEmptyTEnv(),exp.exps));
