...
import { eqTVar, isAtomicTExp, isProcTExp, isTVar, makeProcTExp,makePairTExp, unparseTExp, TExp, TVar, isPairTExp } from "./TExp";
...
export const checkNoOccurrence = (tvar: TVar, te: TExp): true | Error => {
    const check = (e: TExp): true | Error =>
        ...
        isAtomicTExp(e) ? true :
        isPairTExp(e) ? (hasNoError(map(check, [e.leftTE , e.rightTE])) ?
                        true :
                        Error(getErrorMessages(map(check, [e.leftTE , e.rightTE])))) :
...
export const applySub = (sub: Sub, te: TExp): TExp =>
    ...
    isPairTExp(te)? makePairTExp(applySub(sub,te.leftTE),applySub(sub,te.rightTE)):
