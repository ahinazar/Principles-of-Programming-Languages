...
//assert.deepEqual(L5typeof("(cons 1 1): (Pair number number)"), "(Pair number number)");

// LitExp
// assert.deepEqual(infer("(quote ())"), "literal");
// console.log(infer("(quote ())"));

// Pair
// assert.deepEqual(infer("(cons 1 (quote ()))"), "(Pair number literal)");
assert.deepEqual(infer("(cons 1 1)"), "(Pair number number)");
assert.deepEqual(infer("(car (cons 1 1))"), "number");
